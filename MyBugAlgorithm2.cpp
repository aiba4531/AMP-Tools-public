...
amp::Path2D MyBugAlgorithm::plan(const amp::Problem2D& problem) {
    ...
    while (idx < 100) {
        ...
        // Otherwise, start wall-following 
            ...
            double hit_distance = (problem.q_goal - current_point).norm();
            hit_points.push_back(std::make_tuple(current_point, hit_distance));
            ...
            while ((path.waypoints.back() - hit_point).norm() > 0.5 * step_size && n < 150) {
                ...
                //while ((path.waypoints.back() - next_vertex).norm() >  0.5* step_size && m < 150) {
