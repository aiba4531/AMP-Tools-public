...
amp::ManipulatorState MyManipulator2D::getConfigurationFromIK(const Eigen::Vector2d& end_effector_location) const {
    ...
    if (nLinks() == 2) {
        ...
        // Implement inverse kinematics for a 3-link manipulator
        a1 = link_lengths[0];
        a2 = link_lengths[1];
        a3 = link_lengths[2];
        x = end_effector_location[0];
        y = end_effector_location[1];
        //gamma = atan2(y, x); // CITE -- >https://hive.blog/hive-196387/@juecoree/forward-and-reverse-kinematics-for-3r-planar-manipulator
        for (int i = 0; i < 360; i++) {
            gamma = i * M_PI/180;
            x_prime = x - a3*cos(gamma);
            y_prime = y - a3*sin(gamma);

            double costheta2 = (x_prime*x_prime + y_prime*y_prime - a1*a1 - a2*a2)/(2*a1*a2);
            double sintheta2 = sqrt(1 - costheta2*costheta2);

            double sintheta1 = ((a1 + a2*costheta2)*y_prime - a2*sintheta2*x_prime)/(x_prime*x_prime + y_prime*y_prime);
            double costheta1 = ((a1 + a2*costheta2)*x_prime + a2*sintheta2*y_prime)/(x_prime*x_prime + y_prime*y_prime);
            
            joint_angles(0) = atan2(sintheta1, costheta1);
            joint_angles(1) = atan2(sintheta2, costheta2);
            joint_angles(2) = gamma - joint_angles[0] - joint_angles[1];

            // Check if the end effector location forward kinematics matches the input end effector location
            Eigen::Vector2d calculated_end_effector = getJointLocation(joint_angles, nLinks());
            if ((calculated_end_effector - end_effector_location).norm() < 0.001) {
                return joint_angles;
            }
        }

        return joint_angles;
        // Since we know the end effector location we can fig
